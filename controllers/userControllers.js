...
export const register = asyncHandler(async (req, res) => {
  ...
  if (userExists) {
    res.json({ message: `User Already Exists ${email}`});
...
export const googleAuth = asyncHandler(async (req, res) => {
  ...
  const verify = await client.verifyIdToken({
    idToken,
    audience:'820485944799-51ckg1vdqn7132hu3mgbjh0fueqfaoi3.apps.googleusercontent.com',
  });
  console.log(idToken);
  ...
  if (email_verified) {
    ...
    if (user) {
      ...
      const newUser = await User.create({
        ...
        email,
        avatar: picture,
        password: email + '820485944799-51ckg1vdqn7132hu3mgbjh0fueqfaoi3.apps.googleusercontent.com',
      });
      if (newUser) {
        res.json({
          _id: newUser._id,
